...
sample.cov <- matrix(c(0.24, -0.104, -0.0085, -0.207, -0.527, 0.239, -0.104, 3.426, -0.163,
   0.871, 1.203, -0.312, -0.0085, -0.163, 0.412, 0.062, 0.098, 0.103, -0.207, 0.871, 0.062,
   0.893, 0.766, -0.35, -0.527, 1.203, 0.098, 0.766, 1.92, -0.651, 0.239, -0.312, 0.103,
   -0.35, -0.651, 0.466), nrow = 6)
rownames(sample.cov) <- colnames(sample.cov) <- c("lnchla", "pesticide", "macroalgae", "grass", "X1", "x2")
object <- try(sem( sample_cov = sample.cov,
 sample.mean = c(0.4, 0.702, 1.374, 1.044, 2.374, -0.254), sample.nobs = 40, model = Model
), outFile = stdout())
