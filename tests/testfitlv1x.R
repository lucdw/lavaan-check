...
Data <- read.csv("testfitlv1x_dat01.csv")
...
#   idx <- sample(1:length(x), missing.per.var)
#   x[idx] <- NA
#   x
# }))
Data.missing <- read.csv("testfitlv1x_dat02.csv")
...
Data.binary <- read.csv("testfitlv1x_dat03.csv")


fit <- sem(model,
  data = Data.binary, estimator = "WLSMV",
...
#   x
# }))
Data.missing <- read.csv("testfitlv1x_dat04.csv")

# listwise deletion
fit1 <- sem(model,
  data = Data.missing, missing = "listwise", estimator = "WLSMV",
  ordered = c("y1", "y2", "y3", "y4", "y5")
)
# pairwise
fit2 <- sem(model,
  data = Data.missing, missing = "pairwise", estimator = "WLSMV",
  ordered = c("y1", "y2", "y3", "y4", "y5")
)
fit <- fit2
